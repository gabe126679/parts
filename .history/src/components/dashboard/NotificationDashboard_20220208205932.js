...
const NotificationDashboard = (props) => {
        ...
        notifications.map(async item => {
            ...
            data.docs.forEach(doc => {
                if (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value) {
                    ...
                    localStorage.setItem("items", item.id)
                    console.log(seen);
    ...
    const handleNoise = () => {
        console.log(localStorage.getItem('items') === "YAOJ04nUh4gWe6nVa9dC")
        console.log(...seen);
    }
            ...
            {   notifications && notifications.map(item => { 
                ...
                {/* {(() => {
                  (async function() {
                    ...
                    data.docs.forEach(doc => {
                        if (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id)) {
                            return <button onClick={handleClick} value={item.id}> mark as read </button>
                        }
                    });  
                  })()
                })()} */}
