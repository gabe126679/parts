...
class Dashboard extends Component {
  ...
  render() {
    ...
    const handleVote = () => {
      this.props.updateVote(auth.uid);
    }
    const handleClick = () => {
          ...
          {(() => {
              if (
                ...
                return (
                  <div className="col s12 m6">
                    { projects && projects.map(project => {
                        if (project.title === this.state.search || project.content === this.state.search || project.authorFirstName === this.state.search || project.authorLastName === this.state.search || this.state.search === "") {
                          return (
                            <div>
                              <button className="vote btn btn-primary"onClick={handleVote}>  Vote </button>
                              <Link to={'/project/' + project.id}>
                                <ProjectSummary project={project} key={project.id} />
                              </Link>
                            </div>
...
const mapDispatchToProps = dispatch => {
  ...
  return {
    updateVote: (voter) => dispatch(updateVote(voter))
  }
}
