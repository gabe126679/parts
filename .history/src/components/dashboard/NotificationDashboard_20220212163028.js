    ...
    const handleClick = (e) => {
        ...
        notifications.map(async (item) => {
            ...
            data.docs.forEach(async doc => {
                const noteResponse = projectFirestore.collection('notes');
                const noteData = await noteResponse.get()
                noteData.docs.forEach(note => {
                    if (((doc.id === auth.uid || note.data().viewer === auth.uid) && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id)) || ((doc.id === auth.uid || note.data().viewer === auth.uid) && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id) && note.data().notes.includes(item.id))) {
                        seen.push(item.id)
        ...
        if (notifications) {
            notifications.map(async item => {
                ...
                data.docs.forEach(async doc => {
                    ...
                    const noteData = await noteResponse.get()
                    noteData.docs.forEach(note => {
                        // if ((doc.id === auth.uid || note.data().viewer === auth.uid) && (doc.data().viewedUpdates.includes(item.id) || note.data().notes.includes(item.id))) {
                        //     seen.push(item.id)
                        //     props.updateNotification(item)
                        // }
