    ...
    const handleClick = (e) => {
        ...
        notifications.map(async item => {
            ...
            data.docs.forEach(doc => {
                if (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id)) {
                    seen.push(item.id)
    ...
    useEffect(() => {
        if (notifications) {
            notifications.map(async item => {
                ...
                const data = await response.get()
                data.docs.forEach(doc => {
                    if (doc.id === auth.uid && doc.data().viewedUpdates.includes(item.id) && !seen.includes(item.id) && !JSON.parse(localStorage.getItem('items')).includes(item.id)) {
                        seen.push(item.id)
                        localStorage.setItem("items", JSON.stringify(seen))
                    } else if (doc.id === auth.uid && doc.data().viewedUpdates.includes(item.id) && !seen.includes(item.id) && JSON.parse(localStorage.getItem('items')).includes(item.id)) {
                        seen.push(item.id)   
                    } else if (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id)) {
                        seen.pop(item.id)
                        localStorage.setItem("items", JSON.stringify(seen))   
                    }
    ...
    <div className="section">
      <div className="card z-depth-0">
        <div className="card-content">
          ...
          <button onClick={handleNoise}>noise</button>  
