...
const NotificationDashboard = (props) => {
    ...
    const handleClick = (e) => {
        ...
        notifications.map(async item => {
            ...
            data.docs.forEach(doc => {
                if (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id) && JSON.parse(localStorage.getItem('items')).includes(item.id)) {
                    seen.push(item.id)
    ...
    //                     localStorage.setItem("items", JSON.stringify(seen))
    //                 } 
    //                 if (doc.id === auth.uid && (seen.includes(item.id) || JSON.parse(localStorage.getItem('items')).includes(item.id))) {
    //                     seen.push(item.id)
    //                     localStorage.setItem("items", JSON.stringify(seen))
