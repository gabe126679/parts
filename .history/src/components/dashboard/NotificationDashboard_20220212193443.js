...
const NotificationDashboard = (props) => {
    ...
    const handleClick = (e) => {
        ...
        notifications.map(async (item) => {
            ...
            data.docs.forEach(async doc => {
                if ((doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id)) || (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && item.id === e.target.value && !seen.includes(item.id))) {
    ...
    const handleNoise = () => {
        notifications.map(async (item) => {
            ...
            const data = await response.get()
            data.docs.forEach(doc => {
                // if ((doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id)&& !seen.includes(item.id)) || (doc.id === auth.uid && !doc.data().viewedUpdates.includes(item.id) && !seen.includes(item.id))) {
                //     console.log(doc.id);
                // }
                console.log(doc.id === auth.uid)
